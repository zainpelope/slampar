...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query_pengajuan = "INSERT INTO pengajuan_surat (id_pengguna, jenis_surat, status) VALUES ('$id_pengguna', 'Usaha', 'Menunggu Verifikasi')";
    if (mysqli_query($conn, $query_pengajuan)) {
        $id_pengajuan = mysqli_insert_id($conn);

        $query_detail = "INSERT INTO detail_surat (id_pengajuan, nama_lengkap, tempat_lahir, tanggal_lahir, nik, alamat, agama, pekerjaan, keperluan, jenis_usaha, file_pendukung) VALUES ('$id_pengajuan', '$nama_lengkap', '$tempat_lahir', '$tanggal_lahir', '$nik', '$alamat', '$agama', '$pekerjaan', '$keperluan', '$jenis_usaha', '$target_file')";
        mysqli_query($conn, $query_detail);

        echo "<script>alert('Pengajuan Surat Keterangan Usaha berhasil!'); window.location.href='coba.php';</script>";
    } else {
...
<head>
    <title>Form Pengajuan Surat Keterangan Usaha</title>
